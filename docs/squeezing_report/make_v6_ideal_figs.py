"""Build the v6 IDEAL source-limit (QE=1, loss=0) figure under the hardened Ultra
noise model. This is the v6 counterpart of v4's ideal-detection scan
...
hardened model caps the ideal optimum too after the seeded beat-sign correction.

    python docs/squeezing_report/make_v6_ideal_figs.py

Output: docs/squeezing_report/squeezing_frontier_ideal_qe1_v6.png
"""
import sys
...
d = np.load(REPO / "analysis" / "squeezing" / "squeezing_frontier_ideal_v6" / "squeezing_frontier.npz")
...
axM.set_title("Ideal (QE=1, loss=0) v6 hardened xi(Delta,T)")
...
axF.plot(T, best_h, "o-", color="#1f77b4", ms=3, lw=1.5, label="hardened (v6)")
...
sl = oiD
axT.plot(T, Xi[sl, :], "o-", color="#2ca02c", lw=1.6, ms=3, label="hardened (v6)")
...
fig.tight_layout()
fig.savefig(OUT / "squeezing_frontier_ideal_qe1_v6.png", dpi=130)
...
print(f"wrote {OUT / 'squeezing_frontier_ideal_qe1_v6.png'}")
