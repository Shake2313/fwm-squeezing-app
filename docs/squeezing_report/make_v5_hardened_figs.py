"""
Build the v5 hardened-Ultra report figures. The v5 finite-loss frontier is the
SAME grid/solve as v4 (Delta in [-3.5,3.0] GHz x T in [60,150] C, coarse=81,
vstep=5) but with the hardened Ultra noise model now intrinsic to the Ultra path
(conjugate-arm linear OD + pump scatter). This lets the figures show the optimum
move OFF the v4 negative-detuning artifact.

    python docs/squeezing_report/make_v5_hardened_figs.py

Outputs into docs/squeezing_report/ (v5-suffixed):
  * squeezing_frontier_v5.png            -- copy of the v5 scan's 3-panel figure
  * squeezing_frontier_od_v5.png         -- in-cell OD + conjugate OD + xi(T) slices
...
V5 = REPO / "analysis" / "squeezing" / "squeezing_frontier_finite_loss_v5" / "squeezing_frontier.npz"
...
src_png = REPO / "analysis" / "squeezing" / "squeezing_frontier_finite_loss_v5" / "squeezing_frontier.png"
if src_png.exists():
    shutil.copyfile(src_png, OUT / "squeezing_frontier_v5.png")
    print("wrote", OUT / "squeezing_frontier_v5.png")
...
fig.tight_layout()
fig.savefig(OUT / "squeezing_frontier_od_v5.png", dpi=130)
print("wrote", OUT / "squeezing_frontier_od_v5.png")

# 3) v4 legacy vs v5 hardened xi(Delta,T) side by side
if V4.exists():
    d4 = np.load(V4)
    ...
    for ax, XX, ex, oD, oT, DD, TT, xo, tag in [
            ...
            (a5, Xi, extent, oiD, oiT, D, T, xi_opt, "v5 hardened Ultra")]:
    ...
    fig2.savefig(OUT / "squeezing_frontier_hardened_compare_v5.png", dpi=130,
                 bbox_inches="tight")
    print("wrote", OUT / "squeezing_frontier_hardened_compare_v5.png")
